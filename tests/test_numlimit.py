...
def test_numlimit_mkpoint():
    ...
    assert all(point >= limits.low) and all(point <= limits.high), "Generated point is out of bounds."

def test_numlimit_clip():
    limits = NumLimit(low=np.array([0, 0]), high=np.array([5, 5]))
    point = np.array([-1, 6])
    clipped_point = limits.clip(point)
    assert np.array_equal(clipped_point, np.array([0, 5])), "Clip does not work as expected."
